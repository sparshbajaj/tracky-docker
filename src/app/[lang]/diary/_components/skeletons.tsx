	...
	return (
		<header className='rounded-lg border border-muted-foreground/20 p-6 shadow-lg'>
			<h2 className='mb-6 flex items-center text-2xl font-semibold'>
				...
				{dictionary.diary.filters}
			</h2>
			<div className='flex flex-wrap gap-6'>
				<div className='flex-grow space-y-2'>
					<h3 className='mb-2 text-sm font-medium'>
						{dictionary.diary.entryTypes}
					</h3>
					<div className='flex flex-wrap gap-2'>
						{(['meal', 'exercise', 'food', 'updates'] as const).map(type => (
							<Button
								...
								variant='outline'
							>
								{dictionary.diary.types[type]}
							</Button>
						))}
					</div>
				</div>
				<div className='space-y-2'>
					<h3 className='mb-2 text-sm font-medium'>{dictionary.diary.date}</h3>
					<Select>
						<SelectTrigger className='w-[200px]'>
							<Calendar className='mr-2 h-4 w-4' />
							<SelectValue placeholder={dictionary.diary.selectDate} />
						</SelectTrigger>
						<SelectContent>
							<SelectItem value='all'>{dictionary.diary.allDates}</SelectItem>
						</SelectContent>
					</Select>
				</div>
				<div className='space-y-2'>
					<h3 className='mb-2 text-sm font-medium'>
						{dictionary.diary.diaryGroup}
					</h3>
					<Select>
						<SelectTrigger className='w-[200px]'>
							<SelectValue placeholder={dictionary.diary.selectDiaryGroup} />
						</SelectTrigger>
						<SelectContent>
							<SelectItem value='all'>
								{dictionary.diary.allDiaryGroups}
							</SelectItem>
