...
export default function AIChatConversation({
	...
	const persistMessage = useCallback(
		async (message: PersistedMessage) => {
			try {
				...
				toast.error(dictionary.aiChat.errorMessages.unableToSave)
			}
		},
		[applyPersistenceResult, dictionary]
	)
	...
	useEffect(() => {
		...
		const hydrate = async () => {
			try {
				...
				toast.error(dictionary.aiChat.errorMessages.unableToLoad)
			}
		}
		void hydrate()
		return () => {
	...
	}, [updateConversationState, dictionary])
	...
	const handleClearHistory = async () => {
		try {
			...
			setUsageBytes(0)
			setPrunedCount(0)
			toast.success(dictionary.aiChat.successMessages.historyCleared)
		} catch (error) {
			console.error('Error clearing chat history', error)
			toast.error(dictionary.aiChat.errorMessages.unableToClear)
		}
	}
	...
	const handleSend = async () => {
		...
		try {
			...
			toast.error(dictionary.aiChat.errorMessages.unableToSend)
		} finally {
			setLoading(false)
			inputRef.current?.focus()
		}
	...
	const updateMessageContent = async (messageId: string, content: string) => {
		...
		try {
			...
			toast.error(dictionary.aiChat.errorMessages.unableToUpdate)
		}
	}
	...
	const handleFileUpload = async (file: File, defaultMessage: string) => {
		...
		try {
			...
			toast.error(dictionary.aiChat.errorMessages.unableToProcess)
		} finally {
			setLoading(false)
		}
	}
		...
		<>
			<div className='-mt-4 mb-2 flex items-center justify-between'>
				<small className='text-sm text-muted-foreground'>
					{dictionary.aiChat.description}
				</small>
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button type='button' variant='ghost' size='icon'>
							<MoreVertical className='h-4 w-4' />
							<span className='sr-only'>{dictionary.aiChat.chatOptions}</span>
						</Button>
					</DropdownMenuTrigger>
					<DropdownMenuContent align='end' className='w-48'>
						...
						>
							{dictionary.aiChat.clearHistory}
						</DropdownMenuItem>
						<DropdownMenuSeparator />
						<DropdownMenuLabel className='text-xs font-normal text-muted-foreground'>
							{dictionary.aiChat.storageUsage} {usageLabel}
						</DropdownMenuLabel>
					</DropdownMenuContent>
				</DropdownMenu>
			</div>
			{prunedCount > 0 && (
				<div className='mb-2 inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-900 dark:bg-amber-900/30 dark:text-amber-100'>
					{prunedCount === 1
						? dictionary.aiChat.removedMessages.replace(
								'{count}',
								String(prunedCount)
							)
						: dictionary.aiChat.removedMessagesPlural.replace(
								'{count}',
								String(prunedCount)
							)}
				</div>
										...
										>
											...
											{message.image && (
												<Image
													...
													alt={dictionary.aiChat.mealAttachment}
